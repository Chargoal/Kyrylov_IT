    ...
    public partial class mainForm : Form
    {
        ...
        private void SaveToDB()
        {
            foreach (TabPage page in tabControl.TabPages)
            {
                ...
                //dataManager.CurrentDatabase.RemoveTable(dataManager.CurrentDatabase.GetTable(tableName));
                Table table = dataManager.CurrentDatabase.GetTable(tableName);
                table.Entries.Clear();
                DataGridView dgv = (DataGridView)page.Controls.Find("Grid", true)[0];
                foreach (DataGridViewRow row in dgv.Rows)
                ...
                //dataManager.CurrentDatabase.AddTable(table);
            }
        }
        ...
        private void SearchTable(DataGridView table)
        {
            ...
            {
                //DataGridViewRow tempRow = CloneRowWithValues(table, row);
                //foundRows.Insert(row.Index,tempRow);
        ...
        private void SearchSetup()
        {
            foreach (TabPage page in tabControl.TabPages)
            {
                ...
                {
                    searchRow.Cells.Add(new DataGridViewTextBoxCell { Value = "" });
                }
